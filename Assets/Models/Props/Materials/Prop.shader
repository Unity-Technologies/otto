...
Shader "Shader Forge/Prop" {
    Properties {
        _Color ("Color", Color) = (0.25,1,0,1)
        _Metallic ("Metallic", Range(0, 1)) = 0
        _Gloss ("Gloss", Range(0, 1)) = 0.6119658
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
...
///////// Gloss:
                float gloss = 1.0 - ((i.vertexColor.r*(-0.25))+0.7); // Convert roughness to gloss
                float perceptualRoughness = ((i.vertexColor.r*(-0.25))+0.7);
...
////// Specular:
                ...
                float3 specularAO = i.vertexColor.r;
                float LdotH = saturate(dot(lightDirection, halfDirection));
                float3 specularColor = _Metallic;
                float specularMonochrome;
                float3 diffuseColor = (_Color.rgb*i.vertexColor.r*2.0); // Need this for specular when using metallic
...
/////// Diffuse:
                ...
                indirectDiffuse *= i.vertexColor.r; // Diffuse AO
                float3 diffuse = (directDiffuse + indirectDiffuse) * diffuseColor;
////// Emissive:
                float3 emissive = (pow(1.0-max(0,dot(normalDirection, viewDirection)),2.0)*_Color.rgb);
/// Final Color:
